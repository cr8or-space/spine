...
import type { Beat, Content, GenerationRecord } from '@repo/types';
import type { LLMClient } from '@repo/llm';
import { formatContext } from '@repo/llm';
...
export function createGenerationPipeline(client: LLMClient): GenerationPipeline {
   ...
   */
  function formatStructure(structure: import('@repo/types').Structure): string {
    const lines: string[] = [];
