 ...
 * @example
 * ```typescript
 * const characterNameValidator: Validator<CharacterContext> = {
 ...
 *       return [{
 *         status: 'fail',
 *         message: `Duplicate character names: ${duplicates.join(', ')}`,
 *         fix: 'Rename duplicate characters to have unique names',
 *       }];
 *     }
 *     return [{ status: 'pass', message: 'All character names are unique' }];
 *   },
 * };
 * ```
 */
