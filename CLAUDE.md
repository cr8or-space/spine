...
Spine is a framework for building domain-specific authoring tools where:
...
Spine uses LLMs at three isolated levels:
1. **Level 1 (Internal)**: Content generation — LLMs create outlines, drafts, prose
2. **Level 2 (Internal)**: Validation — LLMs perform quality checks, continuity analysis
3. **Level 3 (External)**: Interface — CLI/MCP commands, can be driven by humans or LLMs

The `packages/framework/llm` package powers Levels 1-2. The Level 3 interface LLM (if any) operates outside Spine.
...
- `packages/framework/llm` — LLM client for Levels 1-2 (generation, validation), context assembly
