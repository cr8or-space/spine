...
Spine is designed for LLM-powered authoring where content is generated by LLMs, validation is performed by LLMs, and humans review and approve. See [goals.md](./goals.md) for the three-level LLM architecture that separates interface (external) from generation and validation (internal).
...
│   ├── llm/          # LLM client for Levels 1-2 (generation, validation)
